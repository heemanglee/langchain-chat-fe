---
name: fe-pipeline
description: "프론트엔드 개발 파이프라인. Plan(PRD 인터뷰) → TDD → Build-Fix → Code-Review → Verify 5단계 워크플로우. AskUserQuestion 기반 PRD 생성, 빌드 에러 자동 수정, 품질 검증 포함."
---
...
AskUserQuestion 도구를 사용하여 4라운드에 걸쳐 요구사항을 수집한다.

#### Round 1: 기능 범위 및 목적

| 질문 | header | options |
|------|--------|---------|
| 이 기능의 핵심 목적은 무엇인가? | Purpose | 새 기능 추가, 기존 기능 개선, 버그 수정, 리팩토링 |
| 기능의 범위는? | Scope | 단일 페이지, 여러 페이지, 공통 컴포넌트, 전역 기능 |

#### Round 2: UI/UX 요구사항

| 질문 | header | options |
|------|--------|---------|
| UI 스타일 접근 방식은? | UI Style | 기존 디자인 패턴 따르기, 새로운 디자인 필요, 디자인 시스템 문서 참고, 자유롭게 판단 |
| 반응형 지원 범위는? | Responsive | 데스크톱만, 데스크톱+모바일, 모든 화면 크기 |

#### Round 3: 데이터 및 API

| 질문 | header | options |
|------|--------|---------|
| 백엔드 API 준비 상태는? | API Status | 준비 완료 (openapi.json 참조), 일부만 준비, 아직 미준비 (mock 사용), 프론트엔드 전용 (API 불필요) |
| 상태 관리 방식 선호는? | State Mgmt | Zustand (전역), TanStack Query (서버 상태), useState (로컬), 자동 판단 |

#### Round 4: 우선순위 및 제약사항

| 질문 | header | options |
|------|--------|---------|
| 우선순위가 높은 항목은? (복수 선택) | Priority | 성능 최적화, 접근성(a11y), 애니메이션/전환 효과, 에러 핸들링 |

이후 추가 고려사항은 "Other" 자유 입력으로 수집한다.
...
```typescript
// 공통 래퍼: QueryClientProvider + Router
